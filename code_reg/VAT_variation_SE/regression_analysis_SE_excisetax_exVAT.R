...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
#Load data
reg.data <- import("output_reg_data/se_reg_file_excise_exVAT.csv")      #Load prepared data (Sweden)
...
#OLSrun 0
OLS.reg.0 <- lm(log_gas_cons ~ gas_extaxprice + gas_excise_tax, data = reg.data)
...
test.OLS.reg.0 <- linearHypothesis(model=OLS.reg.0, 
                                   hypothesis.matrix="gas_extaxprice=gas_excise_tax", 
                                   test="F", 
                                   vcov.=NW.reg.0)
p.test.OLS.reg.0 <- round(test.OLS.reg.0[2,4], digits=3)

#OLSrun 1
OLS.reg.1 <- lm(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t ,data = reg.data)         
...
se.reg.1 <- se.reg.1[,2]
test.OLS.reg.1 <- linearHypothesis(model=OLS.reg.1, hypothesis.matrix="gas_extaxprice = gas_excise_tax", test="F", vcov.=NW.reg.1)
p.test.OLS.reg.1 <- round(test.OLS.reg.1[2,4], digits=3)

#OLSrun 2
OLS.reg.2 <- lm(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 ,data = reg.data) 
NW.reg.2 <- NeweyWest(OLS.reg.2, lag=NULL, prewhite = FALSE, adjust=TRUE)
se.reg.2 <- coeftest(OLS.reg.2,vcov=NW.reg.2)
se.reg.2 <- se.reg.2[,2]
test.OLS.reg.2 <- linearHypothesis(model=OLS.reg.2, hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="F", vcov.=NW.reg.2)
p.test.OLS.reg.2 <- round(test.OLS.reg.2[2,4], digits=3)

#OLSrun 3
OLS.reg.3 <- lm(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop ,data = reg.data)
NW.reg.3 <- NeweyWest(OLS.reg.3, lag=NULL, prewhite = FALSE, adjust=TRUE)
se.reg.3 <- coeftest(OLS.reg.3,vcov=NW.reg.3)
se.reg.3 <- se.reg.3[,2]
test.OLS.reg.3 <- linearHypothesis(model=OLS.reg.3, hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="F", vcov.=NW.reg.3)
p.test.OLS.reg.3 <- round(test.OLS.reg.3[2,4], digits=3)          

#OLSrun 4
OLS.reg.4 <- lm(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate ,data = reg.data) 
NW.reg.4 <- NeweyWest(OLS.reg.4, lag=NULL, prewhite = FALSE, adjust=TRUE)
se.reg.4 <- coeftest(OLS.reg.4,vcov=NW.reg.4)
se.reg.4 <- se.reg.4[,2]
test.OLS.reg.4 <- linearHypothesis(model=OLS.reg.4, hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="F", vcov.=NW.reg.4)
...
#IVrun 0  -  (reg4+IVoil) 
IV.reg.0 <- ivreg(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate | 
                                                                   gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + real_oil_price_sek,
                  data=reg.data)
#bwNeweyWest()
...
test.IV.reg.0 <- linearHypothesis(model=IV.reg.0,  hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="Chisq", vcov.=NW.IV.reg.0)
p.test.IV.reg.0 <- round(test.IV.reg.0[2,4], digits=3)

#IV0 - Stats
fs.IV.reg.0 <- lm(gas_extaxprice ~ gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + real_oil_price_sek, data = reg.data)  
...
# IV.reg.1 <- ivreg(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate | 
#                                  gas_extaxprice                          + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + energycarbontax,
#                  data=reg.data)
...
# 
# test.IV.reg.1 <- linearHypothesis(model=IV.reg.1,  hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="Chisq", vcov.=NW.IV.reg.1)
# p.test.IV.reg.1 <- round(test.IV.reg.1[2,4], digits=3)
# 
# #IV1 - Stats
# fs.IV.reg.1 <- lm(gas_excise_tax~gas_extaxprice + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + energycarbontax, data = reg.data)
...
# IV.reg.2 <- ivreg(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate | 
#                                  gas_extaxprice                         + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + gas_energytax,
#                   data=reg.data)
...
# 
# test.IV.reg.2 <- linearHypothesis(model=IV.reg.2,  hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="Chisq", vcov.=NW.IV.reg.2)
# p.test.IV.reg.2 <- round(test.IV.reg.2[2,4], digits=3)
# 
# fs.IV.reg.2 <- lm(gas_excise_tax~gas_extaxprice + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + gas_energytax, data = reg.data)  
...
IV.reg.3 <- ivreg(log_gas_cons ~ gas_extaxprice + gas_excise_tax + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate | 
                    gas_extaxprice + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + real_taxdiff_exVAT,
                data=reg.data)
...
test.IV.reg.3 <- linearHypothesis(model=IV.reg.3,  hypothesis.matrix="gas_extaxprice=gas_excise_tax", test="Chisq", vcov.=NW.IV.reg.3)
p.test.IV.reg.3 <- round(test.IV.reg.3[2,4], digits=3)

fs.IV.reg.3 <- lm( gas_excise_tax~ gas_extaxprice + dummy + t + real_gdp_cap_1000 + urban_pop + unemploymentrate + real_taxdiff_exVAT, data = reg.data)  
...
#                             p.test.IV.reg.2,     #IV2
          ...
          #Labels
          title="Estimation Results from Gasoline Consumption Regressions in Sweden: VAT excluded from all Price Variables",
...
#          star.char ="",          #default:*, if "", not stars shown (only when p-values given)
           out="output_reg_tables/regression_results_SE_excisetax_exVAT.txt"    #for LaTex-file .tex
#          out.header=FALSE
)
