...
#reg.data <- import("output_reg_data/no_reg_file.csv")      #Load prepared data (Norway)
#reg.data <- import("output_reg_data/se_reg_file_excise.csv")      #Load prepared data (Sweden)
#reg.data <- import("output_reg_data/se_reg_file_carbon.csv")      #Load prepared data (Sweden-CarbonTax)
#reg.data <- import("output_reg_data/se_reg_file_excise_exVAT.csv")
#reg.data <- import("output_reg_data/se_reg_file_excise_fullVAT.csv") 
      ...
      fs.IV.reg.3,
       ...
       #Newey-West standart errors
          se = c(list(se.reg.0),  #OLS0 
            ...
            #     list(se.reg.3),  #OLS3 
                 list(se.reg.4),  #OLS4 
                 list(fs.NW.reg.1),  #IV1
                 list(se.IV.reg.1),  #IV1
  #               list(se.IV.reg.2),   #IV2
                list(fs.NW.reg.3),  #IV1
...
star.cutoffs = c(0.05, 0.01),
notes = c("Notes: + p<0.01; * p<0.05")
    #       out="output_reg_tables/regression_results_FI.tex"
#          out.header=FALSE
)
