...
##DATA----------------------------------------------------------------------####
#Load data
reg.data <- import("data/regression_data/se_reg_file_excise.csv")
reg.data <- reg.data[1:38,]                                  
...
#  
 ...
 #First stage regression for endogenous varibales
 fs.IV.reg.3 <- lm(gas_excisetax_with_vat~  t + real_gdp_cap_1000 + urban_pop + unemploymentrate + real_taxdiff, data = reg.data)  
 ...
 #           out="output/regression_tables/regression_results_IV_FI.txt")
...
#          #Not used settings
#          order=c()                 #top to bottom, not all variables need to be mentioned
#          no.space=TRUE,            #remove empty lines
#          omit.stat=c("ser"),       #leave out statistics ("LL"=log-likelyhood, "ser"=residual standart error, "f"=f-statistics)
#          ci=FALSE                  #Confidence interval
#          single.row=FALSE          #Coefficient and CI (or standart error?) in same row
#          p.auto = FALSE,
#          report = "vcstp",
#          report = "vcs",
#          summary = TRUE,
#          omit.stat = "",
#          star.cutoffs=,
#          object.names = "Gasoline consumption",      #shows  the name of regressions (default = FALSE)
star.char = c("*", "+"),
star.cutoffs = c(0.05, 0.01),
notes = c("Notes: + p<0.01; * p<0.05"),
           out="output/regression_tables/regression_results_SE.txt"
#          out.header=FALSE
)
