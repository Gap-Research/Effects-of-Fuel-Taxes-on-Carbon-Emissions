###################################################
### Salience Regression - Sweden (Carbon Tax)
###################################################
...
##DATA----------------------------------------------------------------------####
#Load data
reg.data <- import("data/regression_data/se_reg_file_carbon.csv")     
...
#  
 ...
 # stargazer(fs.IV.reg.1, #fs.IV.reg.2, 
 #           fs.IV.reg.3, se=c(list(fs.NW.reg.1), #list(fs.NW.reg.2), 
 #                                                       list(fs.NW.reg.3)),
 ...
 #           out="output_reg_tables/regression_results_IV_SE_carbon.txt"
 #      )
...
##REGRESSION-TABLE----------------------------------------------------------####
 ...
 stargazer(#Regression runs
    ...
    #Labels
    title="Estimation Results from Gasoline Consumption Regressions for Sweden: Carbon Tax",
    ...
    star.cutoffs = c(0.05, 0.01),
    notes = c("Notes: + p<0.01; * p<0.05"),
    out="output/regression_tables/regression_results_SE_carbon.txt"
    #          out.header=FALSE
 )
