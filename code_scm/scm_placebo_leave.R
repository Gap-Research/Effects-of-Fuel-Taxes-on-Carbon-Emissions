###################################################
### Leave-One-Out: Distribution of the Synthetic Control for Finland & Sweden
###################################################
#--------------------------------HEAD---------------------------------------------------------------#
dev.off()     #Close plot
...
export(dataprep.out$Y1plot, file = paste("output_scm_data/", treat.name, "_co2_cap.csv", sep=""), format = "csv")
export(store, file = paste("output_scm_data/", treat.name, "_leave_one_out_excluded.csv", sep=""), format = "csv")
rm(store)
...
leave.FI <- import("output_scm_data/Finland_leave_one_out_excluded.csv", dec=".", sep=",")   
leave.NO <- import("output_scm_data/Norway_leave_one_out_excluded.csv", dec=".", sep=",")   
leave.SE <- import("output_scm_data/Sweden_leave_one_out_excluded.csv", dec=".", sep=",")   
co2.FI <- import("output_scm_data/Finland_co2_cap.csv", dec=".", sep=",", skip=1)  
co2.NO <- import("output_scm_data/Norway_co2_cap.csv", dec=".", sep=",", skip=1)  
co2.SE <- import("output_scm_data/Sweden_co2_cap.csv", dec=".", sep=",", skip=1)
co2.synth.FI <- import("output_scm_data/Finland_co2_cap_synth.csv", dec=".", sep=",", skip=1)  
co2.synth.NO <- import("output_scm_data/Norway_co2_cap_synth.csv", dec=".", sep=",", skip=1)
co2.synth.SE <- import("output_scm_data/Sweden_co2_cap_synth.csv", dec=".", sep=",", skip=1)

library(dplyr)

year <- 1960:2005
scm.data <- carbontax <- import("output_scm_data/scm_data.csv", dec=".", sep=",") 
...
export(plot.data.FI, file = paste("output_scm_data/Finland_leave_one_out.csv", sep=""), format = "csv")
export(plot.data.NO, file = paste("output_scm_data/Norway_leave_one_out.csv", sep=""), format = "csv")
export(plot.data.SE, file = paste("output_scm_data/Sweden_leave_one_out.csv", sep=""), format = "csv")
...
plot.data.FI <- import("output_scm_data/Finland_leave_one_out.csv", dec=".", sep=",")   
plot.data.NO <- import("output_scm_data/Norway_leave_one_out.csv", dec=".", sep=",") 
plot.data.SE <- import("output_scm_data/Sweden_leave_one_out.csv", dec=".", sep=",") 
...
ggplot(fi.excl.plot) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank())
ggsave("output_scm_plot/Finland_placebo_leave_one_out.png", plot = last_plot(), device = "png", path = NULL,
       scale = 4, 
...
ggplot(no.excl.plot) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank())
ggsave("output_scm_plot/Norway_placebo_leave_one_out.png", plot = last_plot(), device = "png", path = NULL,
       scale = 4, 
       width = 7, height = 5, units = "cm",
       dpi = 300) #, limitsize = TRUE)
...
ggsave("output_scm_plot/Sweden_placebo_leave_one_out.png", plot = last_plot(), device = "png", path = NULL,
       scale = 4, 
       width = 7, height = 5, units = "cm",
       dpi = 300) #, limitsize = TRUE)
#-----------------------------------------------------------------------------------#