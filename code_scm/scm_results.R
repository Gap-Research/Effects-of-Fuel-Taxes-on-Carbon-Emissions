###################################################
### Load file: "output_data/scm_data.csv"
###################################################
###Country id
...
treated <- c(6, 17, 21)
#treated <- 6
#Gasoline-Diesel-Share
library(dplyr)
#carbontax.1 <- mutate(carbontax, share = gas_cons_capita/diesel_cons_capita)
#carbontax.1$share[!is.finite(carbontax.1$share)] <- 0
#carbontax.1 <- mutate(carbontax, fuel_cons_capita = gas_cons_capita+diesel_cons_capita)
#carbontax.2 <- mutate(carbontax.1, gas_share = gas_cons_capita/fuel_cons_capita*100)
...
###################################################
 dataprep.out <-                                             
              dataprep(foo = carbontax,                    
                       #W#Predictor settings
                       predictors = predictor.set1,
                       #                      predictors = c("GDP_per_capita",  "gas_cons_capita", "diesel_cons_capita" ,"vehicles_capita", "urban_pop") ,
...
export(synth.results, file = paste("output_scm_data/", treat.name, "_co2_cap_synth.csv", sep=""), format = "csv")
...
write.table(pred_table, file = paste("output_scm_data/", treat.name, "_tables_predictors.csv", sep=""))
...
#                            )
# rownames(out.pred_table) <- c(predictor.set1, 
#                               "1989", 
#                               "1979",
#                               "1969"
#                               )
...
#          type="text",
          header = FALSE,
         #  flip=TRUE,
         out=paste("output_scm_tables/", treat.name, "_tables_predictors.txt", sep="")
#         out=paste("output_scm_tables/", treat.name, "_tables_predictors.txt", sep="")
)
...
write.table(synth.tables$tab.w[1:13, ], file= paste("output_scm_data/", treat.name, "_tables_country_weights.csv", sep=""))
...
#         type="text",
         ...
         rownames=FALSE,
         #  flip=TRUE,
         out=paste("output_scm_tables/", treat.name, "_tables_country_weights.txt", sep="")
 #        out=paste("output_scm_tables/", treat.name, "_tables_country_weights.txt", sep="")
)
...
png(paste("output_scm_plot/", treat.name, "_path_plot.png", sep=""))   
#cex.lab <- 2.5
#cex.axis <- 2.5
par(cex.axis=1.3)
par(cex.lab=1.3)
...
###################################################
png(paste("output_scm_plot/", treat.name, "_gap_plot.png", sep=""))
par(cex.axis=1.3)
par(cex.lab=1.3)
...
dataprep.out$Y1plot
export(dataprep.out$Y1plot, file = paste("output_scm_data/", treat.name, "_synth_co2.csv", sep=""), format = "csv")
...
export(gaps, file = paste("output_scm_data/", treat.name, "_gaps.csv", sep=""), format = "csv")
export(data.frame(mean.gaps), file = paste("output_scm_data/", treat.name, "_mean_gaps.csv", sep=""), format = "csv")
export(data.frame(mean.percent), file = paste("output_scm_data/", treat.name, "_mean_percent.csv", sep=""), format = "csv")
